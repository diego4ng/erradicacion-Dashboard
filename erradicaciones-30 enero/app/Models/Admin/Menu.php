...
class Menu extends Model
{
    ...
    public function getHijos($padres, $line)
    {
        ...
        foreach ($padres as $line1) {
            if ($line['id'] == $line1['menu_id']) { //Verifica en la lista de padres que a su ves trae hijos si un hijo pertenece a un padre
                $children = array_merge($children, [array_merge($line1, ['submenu' => $this->getHijos($padres, $line1)])]); //Aquì obtiene los nietos
            }
        }
        return $children;
    }
    ...
    public function getPadres($front)
    {
        if ($front) {
            return $this->whereHas('roles', function ($query) {
                $query->where(['role_id'=> session()->get('role_id')],['status'=> 1])->orderby('menu_id');
            })->orderby('menu_id')
                ->orderby('orden')
                ->get()
                ->toArray();
    ...
    public static function getMenu($front = false)
    {
        ...
        $menuAll = [];
        foreach ($padres as $line) { //Solo verifica padres para buscar a sus hijos
            if ($line['menu_id'] != 0) //Si es diferente de 0 ya no es padre y rompe ciclo
                break;
